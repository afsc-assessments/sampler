...
adt <- read_csv("imported/akfin_age.csv",guess_max=50000)
glimpse(adt)
#Get ldt (length data tibble)--------------------------
ldt <- read_csv("imported/akfin_len.csv",guess_max=50000)
glimpse(ldt)
#--------------------------
...
SetBS(n=1000)
 SetBS(n=1)
 i=2016;Sampler(iyr=i,io=T)
for (i in 2021:1991) Sampler(iyr=i)
...
tmp <- cd %>% filter(yr==2020) %>% group_by(bs)  %>% mutate(p=Catch/sum(Catch))
...
p<- ggplot(df.m,aes(x=Year,y=wtstd,color=strata,shape=strata)) + geom_point(size=4) + 
     ...
     scale_x_continuous(breaks=c(seq(2000,2021,2))) +
...
p <- ggplot(df) + 
     geom_hline(yintercept=1,size=1) +
     geom_boxplot(aes(x=Year,y=wtstd,fill=Year),outlier.size=.2,width=.7,notch=TRUE,alpha=.3)  + ylim(c(0.85,1.15)) + xlab('Year') + 
     facet_wrap(~ strata,nrow=3) + ylab('Standardized weight given length') + mytheme + guides(fill="none") 
     p
# Look by length
...
i = 2020
i = 2000
for (i in c(1995,2000,2005,2010,2015 ,2020)) {
...
i=2020
for (i in c(1995,2000,2005,2010,2015 ,2020)) {
  p <- dtmp[yr %between% c(i-4,i) &age>2&age<11] %>% 
  ggplot(aes(x=as.factor(age),y=swt,fill=as.factor(yr-age))) +
       geom_hline(yintercept=1,color="brown",size=1) + scale_fill_discrete(guide=FALSE) +
       geom_violin() + facet_grid(yr~.) + xlab("Age") + ylab("Body weight relative to mean") + 
       mytheme + scale_y_continuous(limits=c(0.7,1.3),breaks=c(0.8,1.0,1.2)) + scale_fill_brewer(palette="Pastel2")    +

       # scale_colour_manual(palette-"Pastel2") #scale_fill_discrete()
  print(p)
  #p <- p + coord_flip()
 #scale_fill_brewer(palette="Pastel2") # + scale_colour_manual(palette-"Pastel2") #scale_fill_discrete()
