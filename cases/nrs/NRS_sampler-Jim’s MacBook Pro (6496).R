...
setwd("~/Onedrive/sampler/cases/yfs")
# source sampler.R
...
SetBS(n=1000)
SetBS(n=1)
for (i in 1991:1994) Sampler(yr=i)
  # missing 1996-1998...
for (i in 1998:2020) Sampler(yr=i)
#--------------------------
ctmp<-wtmp<-NULL
for (i in c(1991:1994,1998:2020)) {
  print(i)
  wtmp <- rbind(wtmp,read_table2(paste0("results/sex_wtage",i,".rep"),col_names=FALSE))
  ctmp <- rbind(ctmp,read_table2(paste0("results/sex_catage",i,".rep"),col_names=FALSE))
}
names(ctmp) <- names(wtmp) <- c("bs","id","year","sex",1:20)
wtage <- wtmp
catage <- ctmp

##################
# Way to write out for flatfish model...
cdf <- pivot_longer(catage,5:24,names_to="age",values_to="catch") %>%
...
wdf <- pivot_longer(wtage,5:24,names_to="age",values_to="weight") %>% 
...
edt %>% filter(type=="N",sex<3,stratum<3) %>% mutate(season=ifelse(stratum==1,"A","B"),sex=ifelse(sex==1,"Male",ifelse(sex==2,"Female","Total"))) %>% group_by(year,sex,season) %>% summarize(Catch=sum(value)) %>% 
...
cds %>% filter(Year==2018) %>% dcast(,Year~age,value.var="cv")
...
data.table(Get_Age(2019)); #adt$yr <- 1991
...
adt
ldt <- data.table(Get_LF(1991)); ldt$yr <- 2016
for (i in 1992:2017) {
for (i in 2018:2020) {
