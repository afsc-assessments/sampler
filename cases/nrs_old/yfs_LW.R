...
# Screen out outliers
summary(df$Length)
df <- filter(df,Length>=25, Length<=50,wt<1.0) %>% group_by(Length) %>% transmute(w_bar_L=mean(wt),wtstd = wt/w_bar_L,wt,month,year,Year=as.factor(year),strata,sex)
dim(df)
...
ggsave(paste0(old_dir,"/figs/lw_anom_str_box.pdf"),p,width=10,height=10)
library(ggthemes)
     df %>% group_by(year) %>% summarise(Mean=mean(wtstd)) %>% ggplot(aes(x=year,y=Mean)) + geom_smooth() + geom_point(color="red",size=4) +
     scale_x_continuous(breaks=c(seq(1990,2020,2))) + ylab("Mean body mass anomaly") +
     geom_hline(yintercept=1,size=1) + mytheme + ylim(c(0.925,1.075)) 


# Look by strata
p <- ggplot(df) + 
     geom_hline(yintercept=1,size=1) +
     geom_boxplot(aes(x=strata,y=wtstd,fill=Year),outlier.size=.2,width=.7,notch=TRUE,alpha=.3)  + ylim(c(0.85,1.15)) + xlab('Year') + 
     ylab('Standardized weight given length') + mytheme + guides(fill=FALSE) 
     p
ggsave(paste0(old_dir,"/figs/lw_anom_str_box.pdf"),p,width=10,height=10)
...
p <- ggplot(df) + 
     geom_hline(yintercept=1,size=1) +
     geom_boxplot(aes(x=Year,y=wtstd,fill=Year),outlier.size=.2,width=.7,notch=TRUE,alpha=.3)  + ylim(c(0.85,1.15)) + xlab('Year') + 
     scale_x_discrete(breaks=c(seq(1996,2020,2))) +
     facet_wrap(~ strata,nrow=3) + 
     ylab('Standardized weight given length') + mytheme + guides(fill=FALSE) 
p
ggsave(paste0(old_dir,"/figs/lw_anom_str_box_yr.pdf"),p,width=10,height=10)
...
p<- ggplot(df.m,aes(x=Year,y=wtstd,color=strata,shape=strata)) + geom_point(size=4) + 
     ...
     geom_hline(yintercept=1,size=1) 
     p
ggsave(paste0(old_dir,"/figs/lw_anom_str_mean.pdf"),p,width=10,height=10)
...
p<- ggplot(df.m,aes(x=Year,y=wtstd,color=month,shape=month)) + geom_point(size=4) + 
     ...
     geom_hline(yintercept=1,size=1) 
     p
ggsave(paste0(old_dir,"/figs/lw_anom_str_mean_month.pdf"),p,width=10,height=10)
...
p<- ggplot(df.m,aes(x=Year,y=wtstd,color=sex,shape=strata)) + geom_point(size=4) + 
     ...
     geom_hline(yintercept=1,size=1) 
p
ggsave(paste0(old_dir,"/figs/lw_anom_str_mean_sex.pdf"),p,width=10,height=10)
...
p
ggsave(paste0(old_dir,"figs/lw_anom_month.pdf",p,width=6,height=4)
...
ggsave(paste0(old_dir,"figs/lw_anom_str.pdf",p,width=6,height=4)
getwd()
old_dir
ggsave(paste0(old_dir,"figs/lw_anom_mo.pdf",p,width=8,height=4)
?geom_violin
